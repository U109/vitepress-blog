...
序列化接口有三个方法，`getSerializerAlgorithm()` 获取具体的序列化算法标识，`serialize()` 将 `Java` 对象转换成字节数组，`deserialize()` 将字节数组转换成某种类型的 `Java` 对象，我们使用最简单的 `json` 序列化方式，使用阿里巴巴的 `fastjson` 作为序列化框架。
